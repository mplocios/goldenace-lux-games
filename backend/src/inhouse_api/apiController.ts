...
export class InHouseController {
  static async init(app: FastifyInstance) {
    ...
    app.get('/inhouse/games', apiController.Games);

    app.post('/initGames', apiController.initGames);
    app.get('/inhouse/getGames', apiController.getGames);
    app.get('/inhouse/saveGames/:page', apiController.saveGames);
    
    app.get('/inhouse/coutGames', apiController.countGames);
    app.get('/inhouse/getLimits', apiController.getLimits);
    ////
    app.get('/inhouse/lobbyGames', apiController.lobbyGames);
    app.get('/inhouse/games/types',apiController.getGameTypes)

    app.post('/inhouse/selfValidate',apiController.selfValidate)
    app.post('/inhouse/test/selfValidate',apiController.selfValidateTest)
  }
}
...
class ApiController {
  ...
  async initGames(req: FastifyRequest<{Body : initGamesParams}>, res: FastifyReply) {
    ...
    try {
      const {game_id , user_details} = req.body
      const api_key = {
        "x-api-key" : "asdasdA1@"
      }

      const { data } = await axios.post(
        `https://gg.kingfisher777.com/api/v1/in-house-games/init-game`,
        {
          game_id,
          user_details,
        },
