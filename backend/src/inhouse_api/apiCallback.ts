...
export class CallbackController {
  static async init(app: FastifyInstance) {
    ...
    app.route({
      method: ["GET", "POST"],
      url: "/callback",
      handler: CallbackHandler.inhouseCallback, 
    });
  }
}
...
export class CallbackHandler {
  ...
  static async inhouseCallback(req: FastifyRequest<{ Body: CasinoCallback }>, res: FastifyReply) {
    try {
...
class TransactionService {
  async balance(data) {
    try {
      ...
      const { player_id, user_id, game_uuid, round_id ,transaction_id} = data;
      ...
      const response = {
        player_id,
        credit : wallet ? wallet.credits : 0,
      }
      return response
    } catch (e) {
  ...
  async win(data) {
    try {
      ...
      const { player_id , game_uuid ,round_id, transaction_bet_id : transaction_id, session_id} = data;
      const amount = parseFloat(data.amount);
      let agent_id = await getAgentId(player_id)
      const agent_info_id = +agent_id.data.data.id
      if(!transaction_id) return { message: "Transaction ID not found." };
