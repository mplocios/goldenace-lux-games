...
export function GameRow({ id, title, subtitle, icon, games, categorySlug }: GameRowProps) {
  return (
    <section id={id} className="space-y-4 animate-fade-in">
      ...
      <div className="-mx-4 flex snap-x snap-mandatory gap-3 overflow-x-auto px-4 pb-2 scrollbar-hide sm:mx-0 sm:px-0 sm:grid sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
        {games.map((g) => (
          <div key={g.id} className="w-40 shrink-0 snap-start sm:w-auto">
            <GameCard game={g} />
          </div>
        ))}
      </div>
