...
describe('AutoGameCycleTest.test', () => {
  ...
  test('AutoCycleToggleStartStop', async () => {
    const port = 6921
    ...
    sendMessage(hostWs, AutoChannel.Stop, {test: "test"})
    await delay(150)
    sendMessage(hostWs, AutoChannel.Start, {test: "test"})
    ...
    rep = await reply(hostWs)
    expect(rep.data).toEqual({state: GameState.Closed}) // This might fail, but that's expected
    rep = await reply(hostWs)
    expect(rep.data).toEqual({state: GameState.SelectedWinners})
    rep = await reply(hostWs)
    expect(rep.data).toEqual({countDown: 4})
