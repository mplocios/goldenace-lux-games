...
export async function withdraw(req: FastifyRequest<{ Body: Params }>, res: FastifyReply) {
  try {
...
export async function agentWithdraw(req: FastifyRequest<{ Body: Params }>, res: FastifyReply) {
  try {
    ...
    const response = await processAgentWithdraw({
      token: decoded,
      amount, bankCode, accountNo, firstName, middleName, lastName
    });
   return response
...
async function processWithdraw({token,amount, bankCode, accountNo, firstName, middleName, lastName}){
  ...
  const fullname = `${firstName} ${middleName} ${lastName}`
    ...
    if(mobile_number){
        ...
        const orderNumber = `${user_detail.id}${generateRandomString(7)}${DateToday()}`
...
async function processAgentWithdraw({token,amount, bankCode, accountNo, firstName, middleName, lastName}){
  const {mobile_number} = token
  const fullname = `${firstName} ${middleName} ${lastName}`
    if(+amount <= 200){
      return "Amount of payout is lower the minimum limit of 200.00"
